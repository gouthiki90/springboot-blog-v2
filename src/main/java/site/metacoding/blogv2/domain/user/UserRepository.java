...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    // @Modifying UPDATE문을 할 때 사용함, excuteUpdate(), 더티체킹 안 해도 됨
    // @Query(value = "UPDATE User SET password = :password, email = :email, addr = :addr WHERE id = :id", nativeQuery = true)
    // void mUpdate(@Param("password") String password, @Param("email") String email, @Param("addr") String addr, @Param("id") Integer id);
}
